...
public class ConfigScreen extends Screen {
	...
	private int row(int index, int rowAmount) {
		int headerSpace = TITLE_PADDING + this.font.FONT_HEIGHT + TITLE_TO_TAB_PADDING + WIDGET_HEIGHT;
	...
	private void createTabButton(int index, int x, int y, int w, int h, String name, Runnable action) {
		this.addButton(new TabButton(index, x, y, w, h, new StringTextComponent(name), button -> action.run()));
	}
	...
	private void createSlider(int x, int y, int w, int h, String name, double min, double max, Supplier<Float> getter, Consumer<Float> setter, Float defaultValue) {
		this.addButton(new AbstractSlider(x, y, w, h, new StringTextComponent(name), normalize(min, max, getter.get())) {
			...
			setMessage(new StringTextComponent(
				String.format("%s: %.2f", name, Math.round(denormalize(min, max, this.field_230683_b_) * 100f) / 100f)
			));
			}
	...
	private void createTab(int index, String name) {
		createTabButton(index, categoryX(index), TITLE_TO_TAB_PADDING + TITLE_PADDING + this.font.FONT_HEIGHT, TAB_WIDTH, WIDGET_HEIGHT, name, () -> {
			switchTab(index);
		});
	}
