...
public class CustomGameRenderer extends GameRenderer {
	...
	public void updateCameraAndRender(float partialTicks, long nanoTime, boolean renderWorldIn) {
		...
		if (!Minecraft.getInstance().skipRenderWorld) {
			...
			int k = Minecraft.getInstance().gameSettings.framerateLimit;
			if (renderWorldIn && Minecraft.getInstance().world != null) {
				...
				if (!Minecraft.getInstance().gameSettings.hideGUI || Minecraft.getInstance().currentScreen != null) {
					...
					Minecraft.getInstance().mainWindow.loadGUIRenderMatrix(Minecraft.IS_RUNNING_ON_MAC);
				...
				Minecraft.getInstance().mainWindow.loadGUIRenderMatrix(Minecraft.IS_RUNNING_ON_MAC);
				// Forge: Fix MC-112292
				net.minecraft.client.renderer.tileentity.TileEntityRendererDispatcher.instance.textureManager = Minecraft.getInstance().getTextureManager();
				// Forge: also fix rendering text before entering world (not part of MC-112292, but the same reason)
				net.minecraft.client.renderer.tileentity.TileEntityRendererDispatcher.instance.fontRenderer = Minecraft.getInstance().fontRenderer;
			}
